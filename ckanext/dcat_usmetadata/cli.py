...
@click.group("dcat-usmetadata")
def dcat_usmetadata():
    """Does DCAT-US metadata setup."""
    pass


@dcat_usmetadata.command("import-publishers")
...
def update_orgs_with_publishers(org, publishers_tree):
    ...
    try:
        ...
        org_metadata = p.toolkit.get_action("organization_show")(
            {"user": user["name"], "ignore_auth": True}, {"id": org}
        )
        ...
        if index_of_publisher_extra:
            org_extras[index_of_publisher_extra]["value"] = json.dumps(publishers_tree)
        else:
            org_extras.append(
