...
class TestDcatUsmetadataPlugin(helpers.FunctionalTestBase):
    ...
    def test_package_creation(self):
        ...
        package_dict = self.app.get(
            "/api/3/action/package_show?id=my_package_000",
    ...
    def test_publisher_load(self):
        ...
        result = runner.invoke(
            cli.import_publishers, [dir_path + "/../publishers.test.csv"]
        )
        self.app = self._get_test_app()
        org = self.app.get(
            "/api/action/organization_show?id=%s" % (self.organization["id"]),
            extra_environ=self.extra_environ,
        )
